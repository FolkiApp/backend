...
@Injectable()
export class UnignoreActivityService {
  private readonly logger = new Logger(UnignoreActivityService.name);

  constructor(private readonly activitiesRepository: ActivitiesRepository) {}

  async execute(user: AuthUser, activityId: number): Promise<void> {
    this.logger.log({
      message: 'Executing unignoreActivity',
      userId: user.id,
      activityId,
    });
    ...
    await this.deleteIgnore(user.id, activity.id);

    this.logger.log({
      message: 'Successfully unignored activity',
      userId: user.id,
      activityId,
    });
  }
  ...
  private async findActivity(activityId: number): Promise<Activity> {
    ...
    try {
      ...
      throw new ActivityUnignoreException();
    }
  ...
  private async deleteIgnore(
    userId: number,
    activityId: number,
  ): Promise<void> {
    try {
      await this.activitiesRepository.deleteIgnore(userId, activityId);
    } catch (error: unknown) {
      this.logger.error({
        message: 'Error deleting activity ignore',
        userId,
        activityId,
        error: error instanceof Error ? error.message : error,
      });
